...
purpose: 在主会话中，根据 problem contract、data spec、references/** 以及知识图谱（若已构建），并结合主会话的领域常识，生成、迭代并通过后端校验的确定性 reference feature extractor。校验通过后，`tools/reference-feature-extractor/**` 才能被 iterative-solving 的 case review 视为可用数值证据来源。
