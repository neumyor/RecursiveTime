...
- `knowledge-to-tools`：由主会话直接负责，不再有独立 Reference Feature Builder 子会话。主会话用其 native 工具读取 task contract、data spec、`references/**`、（若已构建）知识图谱并结合领域常识，先写 `tools/reference-feature-extractor/{evidence-map.json,feature-plan.json}`，再写 `extractor.py`、`manifest.json`、`reference-rules.json`、`README.md`、`test-cases.json`，必须按 `user/data-spec.md` 抽取真实 workspace 样本并包含 control/reference case 评估，写出 `evaluation-report.json`（合成样本只能补充），然后调用 `mcp__ts_harness__validate_reference_feature_extractor` 让后端做强校验（AST 白名单、reference 原文引用、I/O contract、feature plan、evidence map、真实样本 test-cases、evaluation report、重复执行一致性测试）。校验通过后，`extract_reference_features` 和 `inspect_reference_feature_extractor` 才会被后续 session 视为可用数值证据。该 node 通过 `mcp__ts_harness__finish_node` 交还控制权，`outputPaths` 至少要包含上述八个工具文件路径以及 `state/reference-feature-build.json`。
